...
int printspider_generate_waveform(uint16_t *waveform_buffer, const uint16_t *tp, const uint8_t *nozdata, int l) {
    ...
    for (int j = 0; j < 14; j++) {
        ...
        for (int i = 0; i < l; i++) {
            if (is_empty) {
                ...
                set_signals(waveform_buffer, p++, 0);
                continue;
            }
            uint16_t v = tp[i];
            ...
            set_signals(waveform_buffer, p++, v);
        }
