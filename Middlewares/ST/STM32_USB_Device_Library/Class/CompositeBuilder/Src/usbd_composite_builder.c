...
static uint8_t msc_epadd[2];
static uint8_t prnt_epadd[2];
...
/** @defgroup CMPSIT_CORE_Private_FunctionPrototypes
  * @{
  */
 uint8_t USBD_CMPSIT_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
 {
    // MSC = класс 0
    if (USBD_CMPSIT_AddClass(pdev, USBD_MSC_CLASS, CLASS_TYPE_MSC, cfgidx) != USBD_OK)
      return USBD_FAIL;

    // Printer = класс 1
    if (USBD_CMPSIT_AddClass(pdev, USBD_PRNT_CLASS, CLASS_TYPE_PRINTER, cfgidx) != USBD_OK)
      return USBD_FAIL;

    // Инициализируем каждый класс
    for (uint8_t i = 0; i < pdev->NumClasses; i++)
    {
        if (pdev->tclasslist[i].pClass && pdev->tclasslist[i].pClass->Init)
        {
            if (pdev->tclasslist[i].pClass->Init(pdev, cfgidx) != USBD_OK)
                return USBD_FAIL;
        }
    }
...
uint8_t USBD_CMPSIT_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
    for (uint8_t i = 0; i < pdev->NumClasses; i++)
    {
        if (pdev->tclasslist[i].pClass && pdev->tclasslist[i].pClass->DataIn)
        {
            pdev->tclasslist[i].pClass->DataIn(pdev, epnum);
        }
    }
    return USBD_OK;
}

uint8_t USBD_CMPSIT_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  for (uint8_t i = 0; i < pdev->NumClasses; i++)
    {
        if (pdev->tclasslist[i].pClass && pdev->tclasslist[i].pClass->DataOut)
        {
            pdev->tclasslist[i].pClass->DataOut(pdev, epnum);
        }
    }
  return USBD_OK;
}

 uint8_t USBD_CMPSIT_DeInit(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
 {
   // Здесь можно пройтись по pdev->NumClasses и вызвать DeInit у каждого
   for (uint8_t i = 0; i < pdev->NumClasses; i++)
    {
        if (pdev->tclasslist[i].pClass && pdev->tclasslist[i].pClass->DeInit)
        {
            pdev->tclasslist[i].pClass->DeInit(pdev, cfgidx);
        }
    }
   return USBD_OK;
 }
...
{
  USBD_CMPSIT_Init, /* Init, */
  USBD_CMPSIT_DeInit, /* DeInit, */
  USBD_CMPSIT_Setup, /* Setup, */
  NULL, /* EP0_TxSent, */
  NULL, /* EP0_RxReady, */
  USBD_CMPSIT_DataIn, /* DataIn, */
  USBD_CMPSIT_DataOut, /* DataOut, */
  NULL, /* SOF,  */
  NULL,
  NULL,
#ifdef USE_USB_HS
...
uint8_t  USBD_CMPSIT_AddClass(USBD_HandleTypeDef *pdev,
                              ...
                              uint8_t cfgidx)
{
  uint8_t i = pdev->NumClasses;

  if ((pdev->classId < USBD_MAX_SUPPORTED_CLASS) && (pdev->tclasslist[i].Active == 0U))
  {
    /* Store the class parameters in the global tab */
    // pdev->pClass[i] = pclass;
    pdev->tclasslist[i].pClass = pclass;
    pdev->tclasslist[i].ClassId = i;
    pdev->tclasslist[i].Active = 1U;
    pdev->tclasslist[i].ClassType = class;

    switch (class)
    {
      case CLASS_TYPE_MSC:
        pdev->pUserData[i] = &USBD_Storage_Interface_fops_FS;
        break;
...
uint8_t  USBD_CMPSIT_AddToConfDesc(USBD_HandleTypeDef *pdev)
{
  ...
  switch (pdev->tclasslist[pdev->classId].ClassType)
  {
    ...
    case CLASS_TYPE_MSC:
      ...
      /* Set IN endpoint slot */
      pdev->tclasslist[pdev->classId].EpAdd = msc_epadd;
      iEp = pdev->tclasslist[pdev->classId].EpAdd[0] = MSC_EPIN_ADDR;
      USBD_CMPSIT_AssignEp(pdev, iEp, USBD_EP_TYPE_BULK, pdev->tclasslist[pdev->classId].CurrPcktSze);

      /* Set OUT endpoint slot */
      iEp = pdev->tclasslist[pdev->classId].EpAdd[1] = MSC_EPOUT_ADDR;
      USBD_CMPSIT_AssignEp(pdev, iEp, USBD_EP_TYPE_BULK, pdev->tclasslist[pdev->classId].CurrPcktSze);
    ...
    case CLASS_TYPE_PRINTER:
      ...
      /* Set IN endpoint slot */
      pdev->tclasslist[pdev->classId].EpAdd = prnt_epadd;
      iEp = pdev->tclasslist[pdev->classId].EpAdd[0] = PRNT_IN_EP;
      USBD_CMPSIT_AssignEp(pdev, iEp, USBD_EP_TYPE_BULK, pdev->tclasslist[pdev->classId].CurrPcktSze);

      /* Set OUT endpoint slot */
      iEp = pdev->tclasslist[pdev->classId].EpAdd[1] = PRNT_OUT_EP;
      USBD_CMPSIT_AssignEp(pdev, iEp, USBD_EP_TYPE_BULK, pdev->tclasslist[pdev->classId].CurrPcktSze);
...
/**
 ...
 uint8_t *USBD_CMPSIT_GetFSCfgDesc(uint16_t *length)
 {
     if (CurrFSConfDescSz == 0) {
         CMPSIT_BuildFSConfig(); // собрать один раз
     }
     *length = CurrFSConfDescSz;
...
uint8_t USBD_CMPSIT_Setup(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
    uint8_t recip = req->bmRequest & USB_REQ_RECIPIENT_MASK;

    if (recip == USB_REQ_RECIPIENT_INTERFACE)
    {
        uint8_t ifn = (uint8_t)req->wIndex; // номер интерфейса

        // Найти, какому классу принадлежит этот интерфейс
        for (uint8_t i = 0; i < pdev->NumClasses; i++)
        {
            for (uint8_t j = 0; j < pdev->tclasslist[i].NumIf; j++)
            {
                if (pdev->tclasslist[i].Ifs[j] == ifn)
                {
                    // Нашли класс, которому принадлежит интерфейс
                    if (pdev->tclasslist[i].pClass && pdev->tclasslist[i].pClass->Setup)
                    {
                        return pdev->tclasslist[i].pClass->Setup(pdev, req);
                    }
                    return USBD_FAIL;
                }
            }
        }
        return USBD_FAIL; // интерфейс не найден
    }

    // Если запрос не к интерфейсу (например, к endpoint или device) —
    // можно пробросить всем детям, пока кто-то не обработает
    for (uint8_t i = 0; i < pdev->NumClasses; i++)
    {
        if (pdev->tclasslist[i].pClass && pdev->tclasslist[i].pClass->Setup)
        {
            if (pdev->tclasslist[i].pClass->Setup(pdev, req) == USBD_OK)
                return USBD_OK;
        }
    }
...
static const uint8_t PRN_InterfaceDesc[] =
{
  // Interface Descriptor
  0x09,                         // bLength
  USB_DESC_TYPE_INTERFACE,      // bDescriptorType
  0x00,                         // bInterfaceNumber (заполним при вставке)
...
static const uint8_t MSC_InterfaceDesc[] =
{
  // Interface Descriptor
  0x09,                         // bLength
  USB_DESC_TYPE_INTERFACE,      // bDescriptorType
...
static uint16_t USBD_CMPSIT_AppendMSC(uint8_t *dest, uint8_t ifnum)
{
  memcpy(dest, MSC_InterfaceDesc, sizeof(MSC_InterfaceDesc));
  dest[2] = ifnum; // bInterfaceNumber
  return sizeof(MSC_InterfaceDesc);
}

static uint16_t USBD_CMPSIT_AppendPRN(uint8_t *dest, uint8_t ifnum)
{  
  memcpy(dest, PRN_InterfaceDesc, sizeof(PRN_InterfaceDesc));
  dest[2] = ifnum; // bInterfaceNumber
  return sizeof(PRN_InterfaceDesc);
}

void CMPSIT_BuildFSConfig(void)
{
    // Заголовок конфигурации
    USBD_CMPSIT_FSCfgDesc[0] = 0x09; // bLength
    USBD_CMPSIT_FSCfgDesc[1] = USB_DESC_TYPE_CONFIGURATION;
    USBD_CMPSIT_FSCfgDesc[2] = 0x00; // wTotalLength LSB (обновим позже)
    USBD_CMPSIT_FSCfgDesc[3] = 0x00; // wTotalLength MSB
    USBD_CMPSIT_FSCfgDesc[4] = 0x00; // bNumInterfaces (обновим позже)
    USBD_CMPSIT_FSCfgDesc[5] = 0x01; // bConfigurationValue
    USBD_CMPSIT_FSCfgDesc[6] = 0x00; // iConfiguration
    USBD_CMPSIT_FSCfgDesc[7] = 0x80; // bmAttributes (Bus powered)
    USBD_CMPSIT_FSCfgDesc[8] = 0x32; // bMaxPower (100 mA)

    CurrFSConfDescSz = 9;

    // Добавляем MSC интерфейс (IF0)
    CurrFSConfDescSz += USBD_CMPSIT_AppendMSC(&USBD_CMPSIT_FSCfgDesc[CurrFSConfDescSz], 0);

    // Добавляем Printer интерфейс (IF1)
    CurrFSConfDescSz += USBD_CMPSIT_AppendPRN(&USBD_CMPSIT_FSCfgDesc[CurrFSConfDescSz], 1);

    // Обновляем wTotalLength и bNumInterfaces
    USBD_CMPSIT_FSCfgDesc[2] = (uint8_t)(CurrFSConfDescSz & 0xFF);
    USBD_CMPSIT_FSCfgDesc[3] = (uint8_t)(CurrFSConfDescSz >> 8);
    USBD_CMPSIT_FSCfgDesc[4] = 0x02; // MSC + Printer
}

void USBD_CMPSIT_ResetBuild(void)
{
  CurrFSConfDescSz = 0;
  memset(USBD_CMPSIT_FSCfgDesc, 0, sizeof(USBD_CMPSIT_FSCfgDesc));
  // Если ведёшь карту интерфейсов/классов — обнули её тут же
}
